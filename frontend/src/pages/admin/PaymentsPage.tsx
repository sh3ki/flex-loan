    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm sm:px-6 sm:py-5">
          <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">Payments</h1>
          <button 
              ...
              <article key={payment.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="font-mono text-sm font-semibold text-slate-900">{payment.paymentNumber}</p>
                  </div>
                  <p className="text-sm font-semibold text-slate-900">₱{Number(payment.amount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>

                <div className="mt-3 space-y-1.5 text-sm text-slate-700">
                  <p><span className="font-medium text-slate-900">Borrower:</span> {payment.loan?.creditor?.firstName} {payment.loan?.creditor?.lastName}</p>
                  <p><span className="font-medium text-slate-900">Loan:</span> {payment.loan?.loanNumber}</p>
                  <p><span className="font-medium text-slate-900">Date:</span> {new Date(payment.paymentDate).toLocaleDateString()}</p>
                  <p><span className="font-medium text-slate-900">Method:</span> {payment.paymentMethod}</p>
                </div>
